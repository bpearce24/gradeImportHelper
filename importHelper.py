...
def main():
    ...
    if import_type == "C":
        ...
        #print the first 5 students in the grades file with new line between them
        ...
        if len(missing_students) <= 10 and len(missing_students) > 0:
            ...
            for student in missing_students:
                print(student)
        elif len(missing_students) > 10:
            print(
                "There are more than 10 students present in the roster file"
                + " missing from the grades file."
            )
            print("Consider if you want to check the file.")
...
def get_grade_import_type():
    ...
    while True:
        grade_import_type = (
            input("Is this for ProjectStem or CodeHS? (P/C): ").strip().upper()
        )
...
def get_assignment_range(import_type):
    ...
    if import_type == "C":
        ...
        error_message = (
            "Invalid format. Please enter the range in the format X.X.X-X.X.X."
        )
    while True:
...
def check_assignment_range(first_assignment, second_assignment):
    ...
    if first > second:
        print(
            "Invalid assignment range. The first assignment should be less than the second assignment."
        )
        return False
    return True
...
def roster_file_is_valid(roster_file) -> bool:
    ...
    * the student id in the email matches the student id in the unique id"""
    ...
    with open(roster_file, "r") as file:
        reader = csv.reader(file)
        is_valid = (
            validate_headers(file, reader)
            and validate_student_ids(file, reader)
            and validate_student_data(file, reader)
        )
...
def validate_headers(file, reader) -> bool:
    ...
    required_headers = ["Student Email", "First Name", "Last Name", "Unique User ID"]
    ...
    if file_is_missing_headers:
        print(
            "Roster file is missing required headers. Please check the file and try again."
        )
        return False
    if header_row[0].strip() != "Student Email":
        print(
            "Roster file must contain 'Student Email' in the first column of the header row."
        )
        return False
    if header_row[3].strip() != "Unique User ID":
        print(
            "Roster file must contain 'Unique User ID' in the fourth column of the header row."
        )
        return False
    return True
...
def validate_student_data(file, reader) -> bool:
    ...
    for i, row in enumerate(reader):
        if len(row) < 4:
            print(
                f"Missing data in row {i+1}. Roster file should contain 4 columns for each student."
            )
        ...
        if not student_email or not unique_id:
            print(
                f"Missing data in row {i+1}. Email and Unique Id should not be empty."
            )
            print("Please check the file and try again.")
            is_data_missing = True
    if is_data_missing:
        print(
            "Roster file is missing required data. Please check the file and try again."
        )
        return False
    return True
...
def get_CodeHS_graded_assignments(grades_file) -> list[int]:
    with open(grades_file, "r") as file:
        ...
        for i, assignment_type in enumerate(assignment_types):
            if assignment_type.strip().lower() in [
...
def build_CodeHS_csv(roster, grades, graded_assignments, output_file):
    ...
    with open(output_file, "w", newline="") as file:
        ...
        missing_students = []
        # Remove the header from the roster, otherwise the loop will include it and
        # build_CodeHS_row will use 'Student Email' a as key in the grades dictionary.
        del roster["header"]
        for email, student in roster.items():
            if email not in grades:
                missing_students.append(
                    f"Student {student[1]} {student[2]} not found in the grades file."
                )
                continue
            # TODO :THink about adding a verbose flag...
            # print(f"Processing {student[1]} {student[2]}...") 
            # see if the student is in the grades file
...
def build_CodeHS_csv_header(grades, graded_assignments):
    ...
    # Write the header
    header_row = ["Email", "First Name", "Last Name", "Unique User ID"] + assignments
    return header_row


def build_CodeHS_row(student, grades, graded_assignments):
    """Build a row for the student in the CSV file.
        Parameters:
            * student (list): A list of the student data from the roster file.
            * grades (dict): A dictionary of student grades with the email as the key
              and the row as the value. The first row is the header and contains the
              every assignment (including ungraded assignments, it will need to be
              filterd).
            * graded_assignments (list): A list of indices of the assignments that are
              included in the gradebook."""
    row = []
    # Get the email, first name, last name and unique id
    student_email = student[0]
    row.append(student_email)  # email
    row.append(student[1])  # first
    row.append(student[2])  # last
    row.append(student[3])  # unique id
    for i in graded_assignments:
